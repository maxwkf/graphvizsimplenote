...
jQuery(document).ready(function () {
  ...
  function UpdateGraphviz() {
	...
	data = max_ParseGraphviz(data);
    // Generate the Visualization of the Graph into "svg".
    var svg = Viz(data, "svg");
    svg_div.html("<hr>"+svg);
  }
  
  function max_ParseGraphviz(data) {
	var lines = data.split("\n");
	var nodes = [];
	var insertPreviousCodePos = 1;
	for(var i=0; i<lines.length; i++) {
		line = lines[i].trim();
		var name=line.substr(line.indexOf("_")+1,4);
		if (name=="note") {
			// code here
			var currentnode = line.substr(0, line.indexOf("_"));
			var prestr = currentnode + " -> " + currentnode + "_note [dir=none]\n" + 
				"{rank = same; " + currentnode + "; " + currentnode +"_note;}\n";
			lines[i] = prestr + line;
			nodes.push(currentnode +"_note");
		}
		if (line.substr(0, 7) == "digraph") {
			insertPreviousCodePos = i + 1;
		}
		
	};
	lines.splice(insertPreviousCodePos, 0, 
		"node [fontsize = 12, shape = rectangle];" + nodes.join(" ") + ";\n" +
		"node [shape = ellipse, fontsize = 14];\n"
		);
